...
const url = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vStY6NwY6S8eNyF040MrpzrCEQi3R6Ye39OahLdQKFxl5m4wddWonHYFbiPiXRI1Tr2-R4G81p-0BtS/pub?gid=394150613&single=true&output=tsv' // get the live TSV from Google sheets
const fallbackUrl1 = 'https://raw.githubusercontent.com/rosbell/jiscbackup/main/Jisc%20digicap%20resources%20-%20LIVE%20data.tsv' // github periodic backup of the above
const fallbackUrl2 = 'fallback.json' // escholar occasional backup of the above
...
fetch(url).then((response) => {
  if (response.status===200) {
    ...
    throw new Error('google sheet failed to fetch')
  }
})
.then(processResponseJson)
.catch((error) => {
  	console.log('google failed; trying github')
	fetch(fallbackUrl1).then((response) => {
	  if (response.status===200) {
	    return response.json()
	  } else {
	    throw new Error('github failed to fetch')
	  }
	})
	.then(processResponseJson)
	.catch((error) => {
	  	console.log('github failed; trying escholar')
		fetch(fallbackUrl2).then((response) => {
		  if (response.status===200) {
		    return response.json()
		  } else {
		    throw new Error('escholar failed to fetch')
		  }
		})
		.then(processResponseJson)
		.catch((error) => {
		  console.log('escholar failed; giving up 🤦‍')
		  return
		})
	})
})